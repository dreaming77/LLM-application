...
def split_text_by_semantic(text, max_length, model_name, similarity_threshold=0.5):
    """
    基于语义相似度对文本进行分块
...
def save_chunks_to_files(chunks, output_dir):
    ...
    for i, chunk in enumerate(chunks):
        chunk_file_path = os.path.join(output_dir, f"ch_{i + 1}.txt")
...
# 设置输入和输出路径
input_file_path = './dataset/民法典.txt'  # 替换为你的长文本文件路径
output_dir = './dataset/saveChunk/'  # 替换为你希望保存文本块的目录路径
...
# 分割长文本
text_chunks = split_text_by_semantic(long_text, max_length, model_name, similarity_threshold)
